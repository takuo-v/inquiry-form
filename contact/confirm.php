...
if ( $name1 == '' ) {
  $error[ 'name1' ] = '*名前/性は必須項目です。';
  //制御文字でないことと文字数をチェック
} else if ( preg_match( '/\A[[:^cntrl:]]{1,30}\z/u', $name1 ) == 0 ) {
  $error[ 'name1' ] = '*名前/性は30文字以内でお願いします。';
}

if ( $name2 == '' ) {
  $error[ 'name2' ] = '*フリガナ/セイ は必須項目です。';
  //制御文字でないことと文字数をチェック
...
if ( $name3 == '' ) {
  $error[ 'name3' ] = '*名前/名は必須項目です。';
  //制御文字でないことと文字数をチェック
} else if ( preg_match( '/\A[[:^cntrl:]]{1,30}\z/u', $name3 ) == 0 ) {
  $error[ 'name3' ] = '*名前/名は30文字以内でお願いします。';
}

if ( $name4 == '' ) {
  $error[ 'name4' ] = '*フリガナ/メイ は必須項目です。';
  //カタカナ意外でないことのチェック
...
if ( preg_match( '/^[0-9]{3,3}[0-9]{3,4}[0-9]{3,4}$/', $tel ) == 0 ) {
